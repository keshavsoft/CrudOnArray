...
const StartFunc = async ({ inToPath }) => {
    ...
    const LocalVersionNonSecured = await LocalFuncForNonSecureEndPoints({ inToPath });

    if (LocalVersionNonSecured === false) return false;

    StartFuncFromLastRun({
        filePath: `${LocalToPath}/app.js`,
        inNewVersionProtected: LocalVersionSecured,
        newVersion: LocalVersionNonSecured,
